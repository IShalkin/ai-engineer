...
This module owns the BSA/AML, sanctions, and credit-decisioning specifics - the model-classification determination, regulator-set fraud-rate metrics, and CDD/KYC evidence and staleness - while threshold governance and SAR clocks and retention belong to FRD-02.6 and FRD-03.7 - except that changing the value of a threshold already in production routes here, to `FIN-03`, because the required output is the change record and not the detection design - and [fraud-model-risk-guardrails.md](fraud-model-risk-guardrails.md) is canonical for the shared model-risk framework, detection-system design, adverse-action explainability, and agent authority controls, and its citation wins wherever both state the same control.
