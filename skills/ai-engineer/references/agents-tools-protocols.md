...
Session-scoped state needs a deterministic end, not only an expiry policy. On termination, disconnect or timeout, flush file handles, temporary storage, in-memory context and cached tokens; state that outlives the session it belonged to is available to whatever runs next. Per-user data held in a global, a class attribute or a shared singleton is not isolated whatever the retention policy says — instantiate per session, or key every access by session identity. Quota memory, CPU, storage and call rate per session rather than per process, so one session cannot starve the others. `CURRENT_PRIMARY` OWASP, *A Practical Guide for Secure MCP Server Development* v1.0 (February 2026).
...
Building an MCP server and consuming a third-party one are different jobs with different obligations, and a control written for one does not transfer. As consumer, the client is the boundary: discover only through an approved registry, run the server sandboxed with no more filesystem or network reach than the task needs, show the operator the full tool manifest rather than a summary, and grant tool permissions just-in-time with automatic expiry. As builder, the server is the boundary: sign the tool manifest — description, schema, version, required permissions — and verify signature and hash at load; expose to the model only the fields it needs, keeping internal metadata out of model context; and prove the description matches behaviour, flagging any tool that acts outside what it advertises. `CURRENT_PRIMARY` OWASP GenAI Security Project, *A Practical Guide for Securely Using Third-Party MCP Servers* v1.0 (23 October 2025) and *A Practical Guide for Secure MCP Server Development* v1.0 (February 2026).
